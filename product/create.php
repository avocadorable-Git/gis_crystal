...
if ($_SERVER['REQUEST_METHOD']==='POST') {
    ...
    if ($unit_id<=0)             $errors[]="Please select a unit of measure.";
    if (!in_array($status,['active','inactive'])) $errors[]="Invalid status.";
    if (empty($errors)) {
        $stmt=$conn->prepare("INSERT INTO products (name,description,unit_id,status) VALUES (?,?,?,?)");
        $stmt->bind_param("ssis",$name,$description,$unit_id,$status);
        if ($stmt->execute()){$_SESSION['success']="Product created.";header("Location: index.php");exit;}
...
<form method="POST" class="needs-validation" novalidate>
  <div class="form-grid">
    <div class="form-group" style="grid-column:1/-1"><label class="form-label">Product Name <span class="req">*</span></label>
      <input type="text" name="name" class="form-control" maxlength="150" value="<?= htmlspecialchars($name) ?>" placeholder="Enter product name" required>
    ...
    <div class="form-group"><label class="form-label">Unit of Measure <span class="req">*</span></label>
      <select name="unit_id" class="form-select" required>
        ...
        <?php endwhile; ?>
      </select><div class="invalid-feedback">Please select a unit.</div></div>
    ...
    <div class="form-group" style="grid-column:1/-1"><label class="form-label">Description</label>
      <textarea name="description" class="form-control" placeholder="Optional product description"><?= htmlspecialchars($description) ?></textarea></div>
  </div>
  <div style="display:flex;gap:10px;margin-top:8px"><button type="submit" class="btn btn-primary">Save Product</button><a href="index.php" class="btn btn-outline">Cancel</a></div>
</form>
</div></div>
