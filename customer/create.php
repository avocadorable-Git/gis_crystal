...
if ($_SERVER['REQUEST_METHOD']==='POST') {
    ...
    if (empty($errors)) {
        $stmt=$conn->prepare("INSERT INTO customers (name,description,status) VALUES (?,?,?)");
        $stmt->bind_param("sss",$name,$description,$status);
        if ($stmt->execute()) { $_SESSION['success']="Customer created successfully."; header("Location: index.php"); exit; }
