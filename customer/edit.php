...
if ($_SERVER['REQUEST_METHOD']==='POST') {
    ...
    if (empty($errors)) {
        $stmt=$conn->prepare("UPDATE customers SET name=?,description=?,status=? WHERE id=?");
        $stmt->bind_param("sssi",$name,$description,$status,$id);
        if ($stmt->execute()) { $_SESSION['success']="Customer updated."; header("Location: index.php"); exit; }
...
<div class="page-header">
  <div><div class="page-title">Edit Customer</div><div class="page-subtitle">ID #<?= $id ?></div></div>
